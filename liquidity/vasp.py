...
from .custody import Custody, _DEFAULT_ACCOUNT_NAME
from .types.currency import DEFAULT_DIEM_CURRENCY
...
class Vasp:
    ...
    def _add_currency_to_vasp_account(
        self, currency: str, gas_currency: str = DEFAULT_DIEM_CURRENCY
    ) -> None:
        ...
        if currency in [b.currency for b in account_info.balances]:
            print(f"{currency} is already supported by account {self.address_str}")
            return

        script = stdlib.encode_add_currency_to_account_script(
            utils.currency_code(currency)
        )
        ...
        print(
            f"Currency support for {currency} enabled in "
            f"VASP account {self.address_str}"
        )
    ...
    def rotate_dual_attestation_info(
        ...
        new_url: str,
        new_key: bytes,
        gas_currency: str = DEFAULT_DIEM_CURRENCY,
    ) -> None:
