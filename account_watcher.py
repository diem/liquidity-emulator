...
log = logging.getLogger("account-watcher")
...
if run_with_env_vars:
    ...
    network_chainid = int(
        os.getenv("ACCOUNT_WATCHER_CHAIN_ID", default=CHAIN_ID_DEFAULT)
    )
...
if auth_key is None:
    log.error("Must get an authentication key to watch!")
    sys.exit(1)
    ...
    for currency in currencies:
        try:
            ...
            if currency_balance is None:
                log.error(f"Account {watched_account_addr} does not hold {currency}")
                continue

            log.info(f"{currency} balance is {currency_balance}")

            if currency_balance > threshold_amount:
                log.info("Which is enough!")
            else:
                log.info("Need to refill...")

                if network_chainid == testnet.CHAIN_ID.value and private_key is None:
                    log.info(f"Using faucet to get {refill_amount} {currency}")
                    ...
                    log.info(
                        f"Sending transaction from {sender_account_addr_hex} "
            ...
            log.exception(f"Iteration failed; will retry in {sleep_interval} seconds")

    log.info(f"Sleeping for {sleep_interval}")
